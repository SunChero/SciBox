...
Client.prototype.connect = function(retryCount, callback) {
    ...
    self.conn = new WebSocket("wss://ws.hanifi.ca:8097")
    ...
    function handleData(chunk) {
        ...
        //console.log(chunk)
        var message = parseMessage(chunk, self.opt.stripColors);
        try {
    ...
    // self.conn.onclose = console.log(`close event`)
    // self.conn.onerror = console.log(`error event`)
    //self.conn.on('data', handleData);
    
    self.conn.onmessage = ({data}) => handleData(data)
    self.conn.onopen = () => self._connectionHandler()
    ...
    self.conn.onerror =  function(exception) {
